...
| 12 August 2026 | Switched MariaDB task to `ansible.mariadb` collection (eliminates deprecation warning). Added `8021q` kernel module to RHEL 9.7 post-install steps. Removed `commit -w` from `rhel97-modulecleanup.txt` (ramdisk rebuild handled by `rhel97-startup.txt`). Retired `run-54-install-bcm-dvd-local.sh` wrapper — DVD installs now use `ansible-playbook` directly. Removed `00-ssh-keygen-controllernode.sh` — SSH key generation is now manual. Controller-method guide removed. Vault password section corrected to document all three sources including the persisted `.vault_pass` file. |
...
```bash
cd playbooks
ansible-playbook -i inventory/localhost 54-install-bcm-dvd.yaml   # or -i inventory/hosts for the controller method
```
