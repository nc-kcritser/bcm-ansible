...
├── readme-direct-method.md                            # Direct-method quick-start guide
├── files/
│   ├── CM.REPO-goes-here                              # Placeholder — put your downloaded cm.repo here
│   └── cm.repo                                        # BCM yum repo config (credentials; NOT tracked in git)
├── docs/
│   ├── bcm-deployment-guide.md                        # Full deployment guide (both methods)
│   ├── bcm-deployment-guide.docx                      # Word version
│   ├── bcm-deployment-guide-direct.md                 # Direct-method deployment guide
│   ├── bcm-deployment-guide-direct.docx               # Word version
...
└── playbooks/
    ...
    │   ├── cleanup-deployed-image-with-cuda.sh       # Remove firmware/desktop/audio bloat from a BCM image (-i <imagename>)
    │   ├── cleanup-rhel-subsciptions.sh              # Unregister RHEL subscriptions
    │   ├── remove-cuda-from-image.sh                 # Remove NVIDIA/CUDA packages from a BCM image (-i <imagename>)
...
Ansible runs directly on the target head node using the `localhost` inventory. This is the fully tested and recommended deployment path. No separate control node is required.
...
#### Step 1: Set Up Head Node (one-time)
...
Installs EPEL, Ansible collections, and Python packages. Must be run from `playbooks/scripts/` so relative paths resolve correctly.
...
To deploy to a new head node (e.g., `tornado`):

1. **On tornado**, clone this repo or copy the bundle to `/root`.

2. **Run Step 1** (setup) on tornado.
...
|---|---|---|
| `post_install_default_image_archive` | Path on head node to the compute node base image tarball used during BCM install. **Must exist before running playbook 54 or 55.** Created by playbook 20. | `/root/RHEL9-compute-clone.tar.gz` |
...
- **`install_medium: local`** — Install from a local yum repository. Requires `install_medium_local_repo_path` pointing to `/root/cm.repo` (or another path). Playbook 55 runs. Playbook 30 will automatically copy `files/cm.repo` to `/root/cm.repo` if the file is absent on the head node.
...
**Script:** `cd playbooks/scripts && ./run-01-controller-setup.sh`  
**When:** One-time.  
**What:** Installs Ansible, Python packages, and Ansible collections from `prereqs/`.

### Step 2: Prepare Image Capture Target
**Script:** `playbooks/scripts/run-10-prep-captureserver.sh --local`  
...
**Script:** `playbooks/scripts/run-20-grab-host-image.sh --local [-f filename]`  
...
**Script:** `playbooks/scripts/run-30-prep-headnode.sh --local`  
...
- **`cleanup-deployed-image-with-cuda.sh -i <imagename>`** — Removes firmware, desktop packages, audio, and old kernel module trees from a BCM image. Example: `./cleanup-deployed-image-with-cuda.sh -i default-nocuda`.

- **`remove-cuda-from-image.sh -i <imagename>`** — Removes NVIDIA/CUDA packages from a BCM image. Example: `./remove-cuda-from-image.sh -i default-no-cuda`.
...
- **Vault password** is supplied at runtime by `playbooks/scripts/vault-pass-prompt.sh` — from `$ANSIBLE_VAULT_PASSWORD`, `playbooks/.vault_pass` (persisted, mode 600), or an interactive prompt, in that order. Never commit the vault password or `.vault_pass` to the repository.
